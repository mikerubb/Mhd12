...
class CPKProgressHUD: UIVisualEffectView {
    ...
    override func layoutSubviews() {
        ...
        if let indicator = indicator {
            ...
            indicator.frame = CGRect((bounds.width - w) / 2.0, 18.0, w, h)
            textLabel.sizeToFit()
            textLabel.frame = CGRect(0,
                                     indicator.frame.maxY,
                                     bounds.width,
                                     bounds.height - 23.0 - indicator.frame.height)
        }
    }
    ...
    func show(in view: UIView) {
        ...
        self.frame = CGRect((bounds.maxX - bounds.minX - dimension) / 2.0,
                            (bounds.maxY - bounds.minY - dimension) / 2.0,
                            dimension,
                            dimension)
        self.setNeedsLayout()
        self.layoutIfNeeded()
