...
class ATCChatUserSearchViewController: ATCGenericSearchViewController<ATCUser> {
    ...
    static func searchVC(uiConfig: ATCUIGenericConfigurationProtocol,
                         ...
                         viewer: ATCUser,
                         reportingManager: ATCUserReportingProtocol?) -> ATCChatUserSearchViewController {
        let vc = ATCChatUserSearchViewController(configuration: ATCGenericSearchViewControllerConfiguration(searchBarPlaceholderText: "Search friends...",
                                                                                                            uiConfig: uiConfig,
                                                                                                            cellPadding: 0))
        ...
        vc.searchResultsController.selectionBlock = { (navController, object, indexPath) in
            if let user = object as? ATCUser {
                ...
                let threadsVC = ATCChatThreadViewController(user: viewer, channel: channel, uiConfig: uiConfig, reportingManager: reportingManager, recipients: [user])
    ...
    private func sendPushNotification(to user2: ATCUser?) {
        ...
        guard let viewer = viewer else { return }
        let message = "\(viewer.fullName()) sent you a friend request"
...
extension ATCChatUserSearchViewController : ATCMessengerUserAdapterDelegate {
    func userAdapter(_ userAdapter: ATCMessengerUserAdapter, didTapAddUser user: ATCUser) {
        ...
        let hud = CPKProgressHUD.progressHUD(style: .loading(text: "Adding"))
