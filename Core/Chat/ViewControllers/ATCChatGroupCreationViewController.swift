...
class ATCChatGroupCreationViewController: ATCGenericCollectionViewController {
    ...
    init(uiConfig: ATCUIGenericConfigurationProtocol,
        ...
        let emptyViewModel = CPKEmptyViewModel(image: nil,
                                               title: "No Friends",
                                               description: "Add some friends and then create a group.",
                                               callToAction: nil)
    ...
    @objc func okTapped() {
        ...
        let hud = CPKProgressHUD.progressHUD(style: .loading(text: "Creating"))
        hud.show(in: view)
        ATCChatFirebaseManager.createChannel(creator: viewer, friends: selectedFriends) {[weak self] (channel) in
            ...
            if let navController = self.navigationController {
                ...
                channel.participants = self.selectedFriends + [self.viewer]
                let vc = ATCChatThreadViewController(user: self.viewer,
                                                     channel: channel,
                                                     uiConfig: uiConfig,
                                                     reportingManager: self.reportingManager,
                                                     recipients: Array(self.selectedFriends))
    ...
    lazy var okBarButton = UIBarButtonItem(title: "Create Group",
                                           style: .done,
                                           target: self,
